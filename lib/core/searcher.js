...
function parseDynamicCard(dynamic_detail_card) {
    ...
    /* 是否有官方抽奖 */
    if (extend_json.match("lottery_id")) {
...
function parseNewDynamicCard(dynamic_detail_card) {
    const { strToJson } = utils;
    /**临时储存单个动态中的信息 */
    let obj = {};
    const { modules = {}, orig = {}, basic = {}, id_str } = dynamic_detail_card
    const { module_author = {}, module_dynamic = {}, module_more = {}, module_stat = {} } = modules
    const additional = module_dynamic.additional
    const dy_type2chat_type = new Map([[1, 17], [2, 11], [4, 17], [8, 1], [64, 12]]);

    /* 转发者的UID */
    obj.uid = module_author.mid
    /* 转发者的name */
    obj.uname = module_author.uname || ''
    /* 动态是否点过赞 */
    obj.is_liked = module_stat.like.status
    /* 动态的ts10 */
    obj.create_time = module_author.pub_ts
    /* 动态类型 */
    obj.type = orig !== null && orig !== undefined ? 1 : 2
    /* 用于发送评论 */
    obj.rid_str = basic.rid_str.length > 12 ? id_str : basic.rid_str;
    /* 用于发送评论 */
    obj.chat_type = dy_type2chat_type.get(obj.type) || 0;
    /* 转发者的动态ID !!!!此为大数需使用字符串值,不然JSON.parse()会有丢失精度 */
    obj.dynamic_id = id_str;
    /* 定位@信息 */
    obj.ctrl = []
    module_dynamic.desc.rich_text_nodes.forEach(it => {
        if (it.type == 'RICH_TEXT_NODE_TYPE_AT') {
            obj.ctrl.push({
                data: it.rid,
                location: module_dynamic.desc.text.length,
                length: it.orig_text.length + 1,
                type: 1
            })
        }
        /* 是否官方抽奖 */
        if (it.type == 'RICH_TEXT_NODE_TYPE_LOTTERY') {
            obj.hasOfficialLottery = true
        }
    })
    ...
    if (additional !== null && additional !== undefined) {
        ...
        if (additional.type == 'ADDITIONAL_TYPE_RESERVE') {
            const rDesc = ""
            if (additional.reserve.desc3 !== null && additional.reserve.desc3 !== undefined) {
                rDesc = additional.reserve.desc3.text
            }
            obj.reserve_id = additional.reserve.rid + "";
            obj.reserve_lottery_text = additional.reserve.title + " " + rDesc;
        }
...
function modifyDynamicRes(res) {
    ...
    len = len == 0 ? 0 : cards.length
    log.info('处理动态数据', `动态数据读取完毕(${len})(${next.has_more})`);
