...
class Monitor extends Searcher {
    ...
    async go(option) {
        ...
        let
            ...
            { check_if_duplicated, is_copy_chat, copy_blockword, is_repost_then_chat, is_not_create_partition, api_wait } = config;
        ...
        if (uid.length) {
            log.info('关注前',`睡眠`);
            await utils.delay(api_wait * (Math.random() + 0.5));
            await try_for_each(uid, async (u) => {
                ...
                //todo 关注异常时调用其他接口进行重试
                if(status === 4){
                    log.info('关注重试',`睡眠`);
                    await utils.delay(api_wait * (Math.random() + 0.5));
                    status = await bili.reAutoAttention(u);
                    if (status === 6) {
                        status = 0;
                        return false;
                    }
