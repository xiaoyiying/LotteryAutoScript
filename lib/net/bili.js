 ...
 */
function get({ url, config, contents, query, isProxy }) {
    return new Promise((resolve) => {
        ...
        if (hasEnv("ENABLE_PROXY") && isProxy) {
 ...
 * @returns {Promise<string>}
 */
function postDl({ url, config, contents, query }) {
    return new Promise((resolve) => {
        let ro = {
            url,
            method: 'POST',
...
const bili_client = {
    ...
    _getOneDynamicByDyid: new Line(
        ...
        , responseText => {
            ...
            switch (code) {
                case 0:
                    // if (item && item.modules.module_dynamic) {
                    if (card && card.desc) {
                ...
                case 4100024:
                    return [false, undefined, `错误代码:4100024`];
                default:
     ...
     * - 验证码错误 12
     */
    async sendChat(rid, msg, type, code, isDl) {
        let responseText = null;
        if (isDl) {
            responseText = await postDl({
                url: API.REPLY_ADD,
                contents: {
                    oid: rid,
                    type: type,
